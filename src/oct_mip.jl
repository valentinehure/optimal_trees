...
"""
function oct(D::Int64,Nmin::Int64,x::Array{Float64,2},y::Array{Int64,1},K::Int64,alpha::Float64=0.0,C::Int64=0,multi_V::Bool=false,epsilon::Float64=10^(-4),solv::String="C",warm_start::Tree=null_Tree())
    n = length(y)
    p = length(x[1,:])
    ...
    elseif solv == "G"
        ...
        println("Solveur non defini choisir entre CPLEX (C) ou Gurobi (G)")
    end
...
"""
Function solving the MIO problem with quadratic constraints.\n
Arguments :\n
    - D : maximum depth of the resulting tree
    - N_min : minimum number of observation attributed to a leaf
    - x and y : data
...
Result :\n
    ...
    - The gap between the optimal solution and the solution found if the time limit was reached.
"""
function oct_quad(D::Int64,Nmin::Int64,x::Array{Float64,2},y::Array{Int64,1},K::Int64,alpha::Float64=0.0,C::Int64=0,multi_V::Bool=false,epsilon::Float64=10^(-4),epsi_var::Bool=false,warm_start::Tree=null_Tree())
    n = length(y)
    p = length(x[1,:])
